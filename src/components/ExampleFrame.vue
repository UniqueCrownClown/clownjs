  ...
  setup(props) {
    ...
    const activeTitle = computed(() => {
      if (page.value === `${props.mainName}-readme`) {
        ...
        let index = props.exampleList.findIndex(
          (item, index) =>
            page.value === `${props.mainName}-example${index + 1}`
        );
        return index > -1
          ? `${index + 1}.${props.exampleList[index].name}`
          : "";
      }
    });
    ...
    const loadCode = async () => {
      ...
      let codeText = await import(
        `../code/${props.mainName}/${page.value.replace(
          props.mainName + "-",
          ""
        )}.md`
      );
      code.value = codeText.default.replace(
        /\.\.\/\.\.\/\.\.\/\.\.\/packages/g,
        "@clownjs"
      );
    ...
    return {
      ...
      handleResize,
      turnToGithub() {
        window.open("https://github.com/UniqueCrownClown/clownjs");
      },
    };
  },
});
